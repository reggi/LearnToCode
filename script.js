    ...
    var victorycheck = function() {
        if (hero.x <= (exit.x) && exit.x <= (hero.x) && hero.y <= (
            ...
            if (currentlevel < 2) {
                ...
                window.location.href ="http://3d-db.net/learntocode/victory.php";
            }
        }
    };
            ...
            if (hero.direction == "up") {
                if (mapdata[((hero.y / tile.height) - 1) * grid.width +
                    ...
                    if (hero.y > 0) {
                        ...
                        alert("Character has moved off the map. Please look at your code and re run.");
                        keeprunning = false;
                    }
                } else {
                    alert("Character is off the grid. Please look at your code and re run.");
                    keeprunning = false;
                }
            }
        ...
        for (var i = 0, count = timestorepeat; i < count; i++) {
            ...
            tasks.children().each(function(index) {
                ...
                if (task === "if") {
                    var that = $(this);
                    setTimeout(function() {
                        evaluate_if_statement.apply(that);
                ...
                if (task === "while") {
                    iteratecode($(this).find(".sublist"), 10);
                }
            });
            var task;
            //		}
